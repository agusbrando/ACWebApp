...
class YearUnionTest extends TestCase
{
    ...
    public function testUserSergio()
    {
        $subject = Subject::create([
        ...
        $user1 = User::create([
            ...
            'last_name' => 'Test User',
            'email' => 'testUser.test@champusaula.com',
            'password' => bcrypt('password'),
            'role_id' => $role->id,
            'timetable_id' => $timetable->id
        ]);
        ...
        $user2 = User::create([
            'first_name' => 'Test User',
            'last_name' => 'Test User',
            'email' => 'testUser2.lopez@champusaula.com',
            'password' => bcrypt('password'),
            'role_id' => $role->id,
            'timetable_id' => $timetable->id
        ]);

        $yearUnion->users()->attach($user1->id, ['assistance' => true]);
        $yearUnion->users()->attach($user2->id, ['assistance' => true]);
        ...
        //Eliminamos la tabla intermedia
        $yearUnion->users()->detach($user1->id, ['assistance' => true]);
        $yearUnion->users()->detach($user2->id, ['assistance' => true]);
